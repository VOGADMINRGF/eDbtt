...
describe("stream public runtime page", () => {
  it("renders the public participation path and review-first guidance", async () => {
    mocks.buildRuntime.mockResolvedValue({
      ...
      context: {
        ...
        swipesHref: "/swipes?topic=energie-berlin",
        shareEnabled: true,
      },
      participation: {
        openForInput: true,
    ...
    expect(html).toContain("data-testid=\"stream-public-input-panel\"");
    expect(html).toContain("Was passiert mit deinem Beitrag?");
    expect(html).toContain("Wo findest du später Ergebnisse?");
    expect(html).toContain("Zum Anlassraum");
    expect(html).toContain("Zum Dossier");
    expect(html).toContain("Zu Swipes");
    expect(html).toContain("Nichts wird aus dem Stream automatisch als amtliche Wahrheit veröffentlicht.");
