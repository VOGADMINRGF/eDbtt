...
export default function SupportCampaignPage() {
  ...
  async function submitPledge(e: FormEvent) {
    ...
    try {
      ...
      if (!Number.isFinite(amountValue) || amountValue <= 0) throw new Error("Bitte gültigen Betrag eingeben.");
    ...
    <main className="mx-auto flex w-full max-w-4xl flex-col gap-6 px-4 py-10">
      <header className="space-y-2">
        ...
        <p className="text-sm text-[rgb(var(--muted))]">
          {data?.supportCampaign?.description ?? "Hilf mit, dieses Projekt transparent und unabhängig zu tragen."}
        </p>
      </header>
            ...
            {pledgeResult ? (
              <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
                ...
                {pledgeResult.paymentInfo ? (
                  <p className="mt-1">
                    Empfänger: {pledgeResult.paymentInfo.recipient} · IBAN: {pledgeResult.paymentInfo.iban}
