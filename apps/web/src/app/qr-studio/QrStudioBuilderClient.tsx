...
export default function QrStudioPage() {
  ...
  const qrStudioHref = useMemo(
    () => (createdCode ? buildQrStudioCodeHref(createdCode) : null),
    [createdCode],
  );

  useEffect(() => {
    async function renderQr() {
      if (!origin || !qrStudioHref) {
        setQrImage(null);
        return;
      }
      try {
          ...
          <div className="space-y-4">
            <div className="rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5 shadow-sm">
              ...
              <p className="mt-1 text-xs text-[rgb(var(--muted))]">
                Nach dem Erstellen nutzt der QR immer den kanonischen sicheren Einstieg über
                <span className="font-semibold text-[rgb(var(--fg))]"> /qr-studio</span>.
              </p>
              {createdCode ? (
                <div className="mt-4 space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-xs text-[rgb(var(--muted))]">
                      ...
                      {origin && qrStudioHref ? (
                        <a className="underline" href={`${origin}${qrStudioHref}`}>
                          {origin}
                          {qrStudioHref}
                        </a>
                      ) : (
                        <span className="text-rose-600">Der sichere QR-Link wird vorbereitet.</span>
                      )}
