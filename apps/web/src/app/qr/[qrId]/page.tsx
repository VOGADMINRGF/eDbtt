...
function toInternalPath(path: unknown): string | null {
...
export default async function QRScanPage({ params }: PageProps) {
  ...
  if (data.targetType === "dossier") {
    const id = data.targetIds?.[0];
    if (!id) return notFound();
    const anchor =
      typeof data.meta?.anchor === "string" && data.meta.anchor.trim()
        ? `?anchor=${encodeURIComponent(data.meta.anchor.trim())}`
        : "";
    return redirect(`/dossier/${encodeURIComponent(id)}${anchor}`);
  }
  if (data.targetType === "newsroom_companion") {
    const id = data.targetIds?.[0];
    if (!id) return notFound();
    return redirect(`/newsroom/companion/${encodeURIComponent(id)}`);
  }
  if (data.targetType === "custom") {
    const internalPath = toInternalPath(data.meta?.path);
    if (internalPath) return redirect(internalPath);
