    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="vog-chip">
          Feste Optionen: {props.configuredOptionCount}
        </span>
      </div>
        ...
        {props.options.map((option, index) => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              <label
                htmlFor={`manual-anlassraum-option-${index}`}
                className="text-xs font-semibold uppercase tracking-[0.14em] text-[rgb(var(--muted))]"
              >
                Option {index + 1}
              </label>
              <button
                ...
                className="rounded-full border border-[rgb(var(--border))] px-3 py-1 text-xs font-semibold text-[rgb(var(--muted))] transition hover:text-[rgb(var(--fg))] disabled:cursor-not-allowed disabled:opacity-50"
              >
                Entfernen
              </button>
            </div>
            <input
              id={`manual-anlassraum-option-${index}`}
              value={option}
              onChange={(event) => props.onOptionChange(index, event.target.value)}
              ...
              placeholder={index === 0 ? "Zum Beispiel: Ja, jetzt starten" : "Weitere feste Option"}
            />
          </div>
        ))}
      </div>
          ...
          {MANUAL_ANLASSRAUM_COMMUNITY_OPTIONS_MODE_CHOICES.map((choice) => {
            ...
            return (
              <button
                ...
                onClick={() => props.onCommunityOptionsModeChange(choice.value)}
