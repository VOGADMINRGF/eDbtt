...
export type AnlassraumListItem = {
  id: string;
  title: string;
  type: string | null;
  kind: string;
...
export function AdminAnlassraumPageInner({
  initialItemsForTest,
  initialLoadingForTest,
  initialErrorForTest,
}: {
  initialItemsForTest?: AnlassraumListItem[] | null;
  initialLoadingForTest?: boolean;
  initialErrorForTest?: string | null;
} = {}) {
  const { locale } = useLocale();
  const operatorLocale = resolveOperatorLocale(locale);
  const text = getOperatorAnlassraumListTexts(operatorLocale);

  const [items, setItems] = useState<AnlassraumListItem[]>(initialItemsForTest ?? []);
  const [loading, setLoading] = useState(initialLoadingForTest ?? !initialItemsForTest);
  const [error, setError] = useState<string | null>(initialErrorForTest ?? null);
  const [statusFilter, setStatusFilter] = useState<(typeof STATUS_OPTIONS)[number]>("all");
  const [sourceModeFilter, setSourceModeFilter] = useState<(typeof SOURCE_MODE_OPTIONS)[number]>("all");

  useEffect(() => {
    if (initialItemsForTest || initialErrorForTest) return;
    let ignored = false;
    async function load() {
      setLoading(true);
      setError(null);
  ...
  }, [initialErrorForTest, initialItemsForTest, statusFilter, sourceModeFilter]);
  ...
  return (
    <main className="flex w-full flex-col gap-6 py-4">
      <header className="space-y-2">
        ...
        <p className="text-sm text-[rgb(var(--muted))]">{text.headerLead}</p>
      ...
      <section className="overflow-hidden rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] shadow-sm">
        <div className="flex items-center justify-between border-b border-[rgb(var(--border))] px-4 py-3 text-xs text-[rgb(var(--muted))]">
          <span>{formatOperatorNumber(items.length, operatorLocale)} Einträge</span>
          <span>
            {items.length > 0
              ? "Detailpfade sind nur für vorhandene Datensätze verlinkt."
              : "Ohne vorhandene Datensätze gibt es hier keinen künstlich vorgetäuschten Detailpfad."}
          </span>
        </div>
        <table className="min-w-full divide-y divide-[rgb(var(--border))] text-sm">
          ...
          <tbody className="divide-y divide-[rgb(var(--border))]">
            ...
            {!loading && items.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-6 text-center text-[rgb(var(--muted))]">
                  {text.empty} Aktuell gibt es deshalb keinen Detailklick, der eine vorhandene Datenbasis nur vortäuschen würde.
                </td>
              </tr>
            )}
            {!loading &&
...
export default function AdminAnlassraumPageClient() {
  return <AdminAnlassraumPageInner />;
}
