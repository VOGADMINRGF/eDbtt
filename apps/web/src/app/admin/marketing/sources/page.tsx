...
const COPY = {
  de: {
    ...
    guardrail: "Live-Erfassung ist deaktiviert. Es werden keine Demo-Themen oder Fake-Synchronisationen angezeigt.",
    ...
    regionalSourcesDetail: "Region auswählen und dort konkrete Quellen kontrolliert ergänzen oder prüfen.",
    connections: "Konten & Datenquellen",
    connectionsDetail: "API-, CSV-, Secret- und Account-Verbindungen sind noch nicht produktiv freigegeben.",
    topicRadar: "Live-Themenradar",
    topicRadarDetail: "Startet erst nach konkreter Allowlist, Providerentscheidung und Betriebsfreigabe.",
    available: "Verfügbar",
  ...
  en: {
    ...
    guardrail: "Live ingestion is disabled. No demo topics or fake synchronisation is shown.",
    ...
    regionalSourcesDetail: "Select a region and add or check concrete sources in a controlled workflow.",
    connections: "Accounts & data sources",
    connectionsDetail: "API, CSV, secret and account connections are not yet approved for production use.",
    topicRadar: "Live topic radar",
    topicRadarDetail: "Starts only after a concrete allowlist, provider decision and operating approval.",
    available: "Available",
  ...
  return (
    <main className="space-y-6 pb-12" data-testid="admin-marketing-sources-page">
      <header className="rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-5 sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex gap-2 text-xs font-semibold">
            <Link href="/admin/marketing/sources?lang=de" className={languageClass(locale === "de")}>Deutsch</Link>
            <Link href="/admin/marketing/sources?lang=en" className={languageClass(locale === "en")}>English</Link>
          </div>
        </div>
      ...
      <section className="space-y-4" aria-labelledby="source-contract-heading">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[rgb(var(--muted))]">{copy.contract}</p>
          <h2 id="source-contract-heading" className="mt-1 text-2xl font-semibold text-[rgb(var(--fg))]">
            Source- und Providerstatus
          </h2>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <article className="rounded-2xl border border-emerald-300 bg-emerald-50/70 p-4 dark:border-emerald-300/40 dark:bg-emerald-400/10">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-emerald-800 dark:text-emerald-200">{copy.phase1}</p>
          ...
          <article className="rounded-2xl border border-amber-300 bg-amber-50/70 p-4 dark:border-amber-300/40 dark:bg-amber-400/10">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-amber-800 dark:text-amber-200">{copy.phase2}</p>
      ...
      <section className="space-y-4" aria-labelledby="coverage-heading">
        <div>
          <h2 id="coverage-heading" className="text-2xl font-semibold text-[rgb(var(--fg))]">{copy.coverageTitle}</h2>
          <p className="mt-1 max-w-4xl text-sm leading-6 text-[rgb(var(--muted))]">{copy.coverageIntro}</p>
        </div>
        <div className="overflow-hidden rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--card))]">
          <div className="hidden grid-cols-[1.5fr_0.55fr_0.8fr_0.8fr] gap-3 border-b border-[rgb(var(--border))] px-5 py-3 text-xs font-semibold uppercase tracking-[0.1em] text-[rgb(var(--muted))] md:grid">
            <span>{copy.coverageTitle}</span><span>{copy.areas}</span><span>{copy.rawCandidates}</span><span>{copy.operatorTop}</span>
          </div>
          <div className="divide-y divide-[rgb(var(--border))]">
            {model.coverage.map((group) => (
              <article key={group.id} className="grid gap-3 px-5 py-4 md:grid-cols-[1.5fr_0.55fr_0.8fr_0.8fr] md:items-center">
                <h3 className="font-semibold text-[rgb(var(--fg))]">{locale === "de" ? group.labelDe : group.labelEn}</h3>
      ...
      <section className="space-y-4" aria-labelledby="decisions-heading">
        <h2 id="decisions-heading" className="text-2xl font-semibold text-[rgb(var(--fg))]">{copy.decisionsTitle}</h2>
        <div className="grid gap-3 lg:grid-cols-2">
          {model.decisions.map((decision) => (
            <article key={decision.id} className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <h3 className="font-semibold text-[rgb(var(--fg))]">{locale === "de" ? decision.titleDe : decision.titleEn}</h3>
                <DecisionBadge state={decision.state} labels={copy} />
              </div>
              <p className="mt-2 text-sm leading-6 text-[rgb(var(--muted))]">{locale === "de" ? decision.detailDe : decision.detailEn}</p>
            </article>
          ))}
        </div>
      </section>

      <section className="space-y-4 rounded-3xl border border-sky-200 bg-sky-50/60 p-5 dark:border-sky-400/30 dark:bg-sky-400/10" aria-labelledby="work-heading">
        <div>
          <h2 id="work-heading" className="text-2xl font-semibold text-[rgb(var(--fg))]">{copy.workTitle}</h2>
          <p className="mt-1 max-w-4xl text-sm leading-6 text-[rgb(var(--muted))]">{copy.workIntro}</p>
        </div>
        <div className="grid gap-3 lg:grid-cols-3">
          <WorkCard title={copy.regionalSources} detail={copy.regionalSourcesDetail} status={copy.available} href={`${model.regionalSourceRoute}`} />
          <WorkCard title={copy.connections} detail={copy.connectionsDetail} status={copy.notAvailable} />
          <WorkCard title={copy.topicRadar} detail={copy.topicRadarDetail} status={copy.notAvailable} />
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-violet-200 bg-violet-50/60 p-5 dark:border-violet-400/30 dark:bg-violet-400/10">
        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-violet-800 dark:text-violet-200">{copy.nextTitle}</p>
        <p className="mt-2 max-w-4xl text-sm leading-6 text-[rgb(var(--fg))]">{copy.nextDetail}</p>
      </section>

      <Link href={`/admin/marketing?lang=${locale}`} className="inline-flex rounded-xl border border-[rgb(var(--border))] px-4 py-2 text-sm font-semibold text-[rgb(var(--fg))] hover:border-sky-300">
        {copy.backCockpit}
      </Link>
    </main>
  );
...
function Stat({ label, value, tone = "default" }: { label: string; value: string; tone?: "default" | "amber" }) {
  return (
    <article className={`rounded-2xl border p-4 ${tone === "amber" ? "border-amber-300 bg-amber-50/70 dark:border-amber-300/40 dark:bg-amber-400/10" : "border-[rgb(var(--border))] bg-[rgb(var(--card))]"}`}>
...
function Data({ label, value }: { label: string; value: string }) {
  return <p className="text-sm text-[rgb(var(--fg))]"><span className="mr-2 text-xs text-[rgb(var(--muted))] md:hidden">{label}:</span>{value}</p>;
}

function DecisionBadge({ state, labels }: { state: MarketingSourceDecisionState; labels: typeof COPY.de | typeof COPY.en }) {
  const label = state === "decided" ? labels.decided : state === "open" ? labels.open : labels.manualGate;
  const classes = state === "decided"
...
function WorkCard({ title, detail, status, href }: { title: string; detail: string; status: string; href?: string }) {
  const content = (
    <>
      <div className="flex items-center justify-between gap-2"><h3 className="font-semibold text-[rgb(var(--fg))]">{title}</h3><span className="rounded-full border border-[rgb(var(--border))] px-2 py-1 text-xs text-[rgb(var(--muted))]">{status}</span></div>
      <p className="mt-2 text-sm leading-6 text-[rgb(var(--muted))]">{detail}</p>
      {href ? <p className="mt-3 text-sm font-semibold text-sky-700 dark:text-sky-300">Öffnen →</p> : null}
    </>
  );
  return href ? <Link href={href} className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4 hover:border-sky-300">{content}</Link> : <article className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4 opacity-80">{content}</article>;
}
