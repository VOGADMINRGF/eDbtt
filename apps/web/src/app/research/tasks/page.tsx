  ...
  return (
    <main className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-8">
      <header className="space-y-1">
        <p className="text-xs font-semibold uppercase tracking-wide text-[rgb(var(--muted))]">Community Research</p>
        <h1 className="text-2xl font-bold text-[rgb(var(--fg))]">Research Board</h1>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] shadow-sm">
          <div className="border-b border-[rgb(var(--border))] px-4 py-3">
            <h2 className="text-sm font-semibold text-[rgb(var(--fg))]">Offene Aufgaben</h2>
            <p className="text-xs text-[rgb(var(--muted))]">Basic / Advanced / Expert</p>
          </div>
          <div className="divide-y divide-[rgb(var(--border))]">
            {loading ? (
              <div className="px-4 py-4 text-sm text-[rgb(var(--muted))]">Lädt …</div>
            ) : openTasks.length === 0 ? (
              <div className="px-4 py-4 text-sm text-[rgb(var(--muted))]">Keine offenen Aufgaben.</div>
            ) : (
              openTasks.map((task) => (
                <button
                  key={task.id}
                  onClick={() => selectTask(task.id!)}
                  className={`flex w-full flex-col items-start gap-1 px-4 py-3 text-left text-sm hover:bg-[rgb(var(--bg))] ${
                    selectedId === task.id ? "bg-[rgb(var(--bg))]" : ""
                  }`}
                >
                  <div className="flex w-full items-center justify-between">
                    <span className="font-semibold text-[rgb(var(--fg))]">{task.title}</span>
                    <span className="rounded-full bg-[rgb(var(--bg))] px-2 py-0.5 text-[11px] font-semibold uppercase text-[rgb(var(--muted))]">
                      {task.level ?? "basic"}
                    </span>
                  </div>
                  {task.tags?.length ? (
                  ...
                  {task.description && <p className="text-xs text-[rgb(var(--muted))]">{task.description}</p>}
                </button>
              ))
            )}
          </div>
        ...
        <div className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] shadow-sm">
          <div className="border-b border-[rgb(var(--border))] px-4 py-3">
            <h2 className="text-sm font-semibold text-[rgb(var(--fg))]">Task-Details & Beitrag</h2>
            {!selectedTask && <p className="text-xs text-[rgb(var(--muted))]">Wähle eine Aufgabe aus der Liste.</p>}
            {selectedTask && <p className="text-xs text-[rgb(var(--muted))]">Level: {selectedTask.level ?? "basic"}</p>}
          </div>

          {selectedTask ? (
            <form className="space-y-4 px-4 py-4" onSubmit={submitContribution}>
              <div>
                <h3 className="text-base font-semibold text-[rgb(var(--fg))]">{selectedTask.title}</h3>
                {selectedTask.description && <p className="text-sm text-[rgb(var(--muted))]">{selectedTask.description}</p>}
              ...
              <div className="space-y-1">
                <label className="text-sm font-semibold text-[rgb(var(--muted))]">Kurzfassung</label>
                <input
              ...
              <div className="space-y-1">
                <label className="text-sm font-semibold text-[rgb(var(--muted))]">Details / Quellen</label>
                <textarea
                  className="w-full rounded-lg border border-[rgb(var(--border))] px-3 py-2 text-sm focus:border-[rgb(var(--border))] focus:outline-none"
                  rows={4}
                  value={details}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-sm font-semibold text-[rgb(var(--muted))]">Links / Quellen</label>
                  <button
                    type="button"
            ...
            <div className="px-4 py-4 text-sm text-[rgb(var(--muted))]">Keine Aufgabe ausgewählt.</div>
          )}
        </div>
      </div>
    </main>
