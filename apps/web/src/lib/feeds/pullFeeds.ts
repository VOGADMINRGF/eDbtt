...
function parseRss(xml: string): ParsedArticle[] {
  ...
  for (const item of items) {
    ...
    out.push({
      ...
      summary: desc
        ? unescapeXml(desc).replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 800)
        : null,
...
function parseAtom(xml: string): ParsedArticle[] {
  ...
  for (const entry of entries) {
    ...
    out.push({
      ...
      url: unescapeXml(link).trim(),
      summary: summary
        ? unescapeXml(summary).replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().slice(0, 800)
        : null,
...
async function processFeed(
  ...
  const errors: string[] = [];
  try {
    const items = (await fetchAndParseFeed(ref.feedUrl, opts.fetchTimeoutMs)).slice(
      0,
...
export async function pullFeeds(raw: PullFeedsOptions = {}): Promise<PullFeedsResult> {
  const scope = raw.scope === "global" ? "global" : "de";
  const maxFeeds = Math.max(1, Math.min(100, Number(raw.maxFeeds ?? 20) || 20));
  const maxItemsPerFeed = Math.max(1, Math.min(50, Number(raw.maxItemsPerFeed ?? 12) || 12));
  const dryRun = Boolean(raw.dryRun);
  const regionCode =
    typeof raw.regionCode === "string" && raw.regionCode.trim() ? raw.regionCode.trim() : null;
  const fetchTimeoutMs = Math.max(
    2_000,
    Math.min(
      30_000,
      Number(process.env.FEEDS_PULL_TIMEOUT_MS ?? raw.fetchTimeoutMs ?? DEFAULT_FETCH_TIMEOUT_MS) ||
  ...
  const feedConcurrency = Math.max(
    1,
    Math.min(
      MAX_FEED_CONCURRENCY,
      Number(
        process.env.FEEDS_PULL_CONCURRENCY ?? raw.feedConcurrency ?? DEFAULT_FEED_CONCURRENCY,
      ) || DEFAULT_FEED_CONCURRENCY,
    ),
  );

  const { config: cfg, searched, source } = await loadFeeds(scope);
  if (!cfg) {
    return { ok: false, error: "feeds_config_missing", scope, searched };
  }

  const collected = collectFeedRefs(cfg);
  const regionFilter = filterFeedRefsByRegion(collected, regionCode);
  const feedRefs = (regionCode && !regionFilter.isGlobal ? regionFilter.feedRefs : collected).slice(
    0,
    maxFeeds,
  );
  if (regionCode && !regionFilter.isValid) {
    return { ok: false, error: "invalid_region", scope, regionCode };
  }
  ...
  return {
    ...
    skippedExisting,
    errors,
    configSource: process.env.NODE_ENV !== "production" ? source ?? null : undefined,
    feedRefs: process.env.NODE_ENV !== "production" ? feedRefs.length : undefined,
  };
}
