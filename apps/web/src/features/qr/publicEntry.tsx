...
export async function renderResolvedQrCodeEntry(qrId: string) {
  const normalizedQrId = validateQrCodeValue(qrId);
  if (!normalizedQrId) {
    return <QrFallback qrId={qrId} reason="invalid_target" />;
  }
  ...
  try {
    const res = await fetch(`${base}/api/qr/resolve?qrId=${encodeURIComponent(normalizedQrId)}`, {
      cache: "no-store",
    });
  ...
  if (data.targetType === "statement") {
    ...
    if (!id) return <QrFallback qrId={normalizedQrId} reason="invalid_target" />;
    redirect(`/statements/${encodeURIComponent(id)}`);
  }
  if (data.targetType === "contribution") {
    const id = data.targetIds?.[0];
    if (!id) return <QrFallback qrId={normalizedQrId} reason="invalid_target" />;
    redirect(`/contribute?source=qr&target=${encodeURIComponent(id)}`);
  }
  if (data.targetType === "stream") {
    const id = data.targetIds?.[0];
    if (!id) return <QrFallback qrId={normalizedQrId} reason="invalid_target" />;
    redirect(`/stream/${encodeURIComponent(id)}`);
  }
...
export function renderResolvedQrTargetEntry(
  rawTarget: string,
  options: { caller?: string | null } = {},
) {
  ...
  if ("message" in validation) {
    return (
      <InvalidQrTarget
        ...
        callerLabel={getQrStudioCallerLabel(options.caller)}
      />
    );
  }

  const target = validation.value;
  const callerLabel = getQrStudioCallerLabel(options.caller);
  const openHref =
  ...
  return (
    ...
    >
      ...
      <section className="rounded-2xl border border-rose-200 bg-rose-50/80 p-4 text-sm text-rose-900 shadow-sm">
        <p className="text-xs">Aufrufer: {callerLabel}</p>
