import type { VoxyRenderUploadTargetPolicyPanelModel } from "@/features/create/voxyRenderUploadTargetPolicyContract";

type VoxyRenderUploadTargetPolicyPanelProps = {
  model: VoxyRenderUploadTargetPolicyPanelModel | null;
  title?: string;
  dataTestId?: string;
};
...
export default function VoxyRenderUploadTargetPolicyPanel({
  model,
  title,
  dataTestId,
}: VoxyRenderUploadTargetPolicyPanelProps) {
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <StatusChip label={model.uploadTargetPolicyStatusLabel} muted />
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-6">
        {[
          {
            title: "Noch kein Upload",
            summary: "upload_target bleibt explizit getrennt von uploaded.",
          },
          {
            title: "Kein Storage-Write",
            summary: "storage_policy beschreibt nur Anforderungen und schreibt nichts.",
          },
          {
            title: "Keine Signed URL",
            summary: "Signed Access Candidate erzeugt keine Signed URL.",
          },
          {
            title: "Keine Public URL",
            summary: "Es gibt keine Public Base URL und keinen öffentlichen Asset-Link.",
          },
          {
            title: "Kein Delete Job",
            summary: "Retention- und Deletion-Policy bleiben ohne Job-Ausführung.",
          },
          {
            title: "Keine Veröffentlichung",
            summary: "uploaded bleibt ungleich published, social_posted und scheduled.",
          },
        ].map((card) => (
          <div
            key={card.title}
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <TextList
          title="Upload Target"
          items={[model.uploadTargetLine]}
          emptyLabel="Noch kein Upload-Ziel sichtbar."
        />
        <TextList
          title="Access Policy"
          items={[model.accessPolicyLine, model.signedAccessLine]}
          emptyLabel="Noch keine Access-Policy sichtbar."
        />
        <TextList
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        <TextList
          ...
          items={[model.retentionPolicyLine]}
          emptyLabel="Noch keine Retention-Policy sichtbar."
        />
        <TextList
          title="Deletion Policy"
          items={[model.deletionPolicyLine]}
          emptyLabel="Noch keine Deletion-Policy sichtbar."
        />
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-4 py-3">
        ...
        <p className="mt-3 text-xs leading-5 text-[rgb(var(--muted))]">
          Media-/Storage-Wahrheit: {model.commandPreview.mediaStorageTruthId ?? "noch keine Persistenz"}
        </p>
        <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
          Vorschau erstellt: {model.commandPreview.createdAt ?? "nur als Readmodel-Vorschau"}
        </p>
        <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
          Status: {model.commandPreview.uploadTargetPolicyStatusLabel} · {model.commandPreview.nextStepLabel}
        </p>
        {model.latestRecord ? (
          <>
            <p className="mt-3 text-sm font-medium text-[rgb(var(--fg))]">
              Letzte persistierte Upload-Target-Policy
            </p>
            <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
              {model.latestRecord.uploadTargetPolicyStatusLabel}
              {model.latestRecord.uploadTargetPolicyVersion
                ? ` · Version ${model.latestRecord.uploadTargetPolicyVersion}`
                : ""}
            </p>
            <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
              Media-/Storage-Wahrheit: {model.latestRecord.mediaStorageTruthId ?? "keine"}
            </p>
            <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
          ...
          <p className="mt-3 text-xs leading-5 text-[rgb(var(--muted))]">
            Noch keine persistierte Upload-Target-Policy sichtbar.
          </p>
        )}
      </div>
    </section>
