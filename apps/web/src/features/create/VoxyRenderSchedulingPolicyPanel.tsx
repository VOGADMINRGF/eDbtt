import type { VoxyRenderSchedulingPolicyPanelModel } from "@/features/create/voxyRenderSchedulingPolicyContract";

type VoxyRenderSchedulingPolicyPanelProps = {
  model: VoxyRenderSchedulingPolicyPanelModel | null;
  title?: string;
  dataTestId?: string;
};
...
export default function VoxyRenderSchedulingPolicyPanel({
  model,
  title,
  dataTestId,
}: VoxyRenderSchedulingPolicyPanelProps) {
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <StatusChip label={model.schedulingPolicyStatusLabel} muted />
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-6">
        {[
          {
            title: "Noch nicht geplant",
            summary: "schedule_candidate bleibt explizit getrennt von scheduled.",
          },
          {
            title: "Kein Scheduler Job",
            summary: "scheduling_policy beschreibt nur spätere Regeln und startet keinen Job.",
          },
          {
            title: "Kein Kalendertermin",
            summary: "calendar_hint erzeugt weder Event noch Reminder.",
          },
          {
            title: "Kein Posting-Zeitpunkt",
            summary: "distribution_time bleibt Kandidat und nicht posted_at.",
          },
          {
            title: "Keine Veröffentlichung",
            summary: "publish_window bleibt getrennt von published und social_posted.",
          },
          {
            title: "Kein Auto-Publish",
            summary: "Es wird nichts geplant, gepostet oder veröffentlicht.",
          },
        ].map((card) => (
          <div
            key={card.title}
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <TextList
          ...
          items={[model.scheduleCandidateLine]}
          emptyLabel="Noch kein Schedule-Kandidat sichtbar."
        />
        <TextList
          title="Publish Window"
          items={[
            model.publishWindowLine,
            model.timezonePolicyLine,
            model.platformTimingPolicyLine,
          ]}
          emptyLabel="Noch kein Publish Window sichtbar."
        />
        <TextList
          title="Calendar Hint"
          items={[model.calendarHintLine]}
          emptyLabel="Noch kein Kalenderhinweis sichtbar."
        />
      </div>
      ...
      <div className="mt-4 rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-4 py-3">
        ...
        <p className="mt-3 text-xs leading-5 text-[rgb(var(--muted))]">
          Upload-Target-Policy: {model.commandPreview.uploadTargetPolicyId ?? "noch keine Persistenz"}
        </p>
        <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
          Vorschau erstellt: {model.commandPreview.createdAt ?? "nur als Readmodel-Vorschau"}
        </p>
        <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
          Status: {model.commandPreview.schedulingPolicyStatusLabel} · {model.commandPreview.nextStepLabel}
        </p>
        {model.latestRecord ? (
          <>
            <p className="mt-3 text-sm font-medium text-[rgb(var(--fg))]">
              Letzte persistierte Scheduling-Policy
            </p>
            <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
              {model.latestRecord.schedulingPolicyStatusLabel}
              {model.latestRecord.schedulingPolicyVersion
                ? ` · Version ${model.latestRecord.schedulingPolicyVersion}`
                : ""}
            </p>
            <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
              Upload-Target-Policy: {model.latestRecord.uploadTargetPolicyId ?? "keine"}
            </p>
            <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
          ...
          <p className="mt-3 text-xs leading-5 text-[rgb(var(--muted))]">
            Noch keine persistierte Scheduling-Policy sichtbar.
          </p>
        )}
      </div>
    </section>
