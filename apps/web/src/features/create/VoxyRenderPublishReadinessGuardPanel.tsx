import type {
  VoxyRenderPublishReadinessGuardPanelModel,
} from "@/features/create/voxyRenderPublishReadinessGuardContract";

type VoxyRenderPublishReadinessGuardPanelProps = {
  model: VoxyRenderPublishReadinessGuardPanelModel | null;
  title?: string;
  dataTestId?: string;
};
...
export default function VoxyRenderPublishReadinessGuardPanel({
  model,
  title,
  dataTestId,
}: VoxyRenderPublishReadinessGuardPanelProps) {
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <StatusChip label={model.guardStatusLabel} muted />
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-7">
        {[
          {
            title: "Noch nicht veröffentlichungsbereit",
            summary: "Dieser Guard beschreibt nur Blocker und startet keine Veröffentlichung.",
          },
          {
          ...
          },
          {
            ...
            summary: "Selbst eine spätere Freigabe wäre noch keine Veröffentlichung.",
          },
          {
            title: "Kein Upload",
            summary: "Der Guard erzeugt weder Upload noch Datei noch URL.",
          },
          {
            title: "Kein Social Posting",
            summary: "Es gibt keinen Social API Call und keinen Post.",
          },
          {
            title: "Kein Scheduling",
            summary: "Es entsteht kein Termin, kein Schedule und kein Job.",
          },
          {
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-4 py-3">
          ...
          <p className="mt-3 text-xs leading-5 text-[rgb(var(--muted))]">
            Outcome Handoff:{" "}
            {model.commandPreview.previewOutcomeHandoffId ?? "noch keine Persistenz"}
          </p>
          <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
            Vorschau erstellt: {model.commandPreview.createdAt ?? "nur als Readmodel-Vorschau"}
          </p>
          <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
            Status: {model.commandPreview.guardStatusLabel} · {model.commandPreview.nextStepLabel}
          </p>
          {model.latestRecord ? (
            <>
              <p className="mt-3 text-sm font-medium text-[rgb(var(--fg))]">
                Letzter persistierter Publish-Readiness-Guard
              </p>
              <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
                {model.latestRecord.guardStatusLabel}
                {model.latestRecord.guardVersion ? ` · Version ${model.latestRecord.guardVersion}` : ""}
              </p>
              <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
                Outcome Handoff: {model.latestRecord.previewOutcomeHandoffId ?? "keiner"}
              </p>
              <p className="mt-1 text-xs leading-5 text-[rgb(var(--muted))]">
            ...
            <p className="mt-3 text-xs leading-5 text-[rgb(var(--muted))]">
              Noch kein persistierter Publish-Readiness-Guard sichtbar.
            </p>
          )}
        </div>
      </div>
