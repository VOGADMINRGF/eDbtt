...
${buildVoxyHeadRelativeFaceRigCss(headFrame)}</style></head><body><main class="viewport" data-format="${format}" data-frame-index="${input.displayFrameIndex ?? frameIndex}" data-source-frame-index="${frameIndex}" data-at-ms="${state.atMs.toFixed(3)}" data-waveform-count="1" data-waveform-placement="behind_voxy" data-waveform-audio-reactive="${input.waveformAmplitude !== undefined}" data-waveform-amplitude="${waveformAmplitude.toFixed(4)}" data-mouth-state="${state.mouthState}" data-mouth-next-state="${state.mouthNextState}" data-mouth-mix="${state.mouthMix}" data-hand-gesture="${state.gesture}" data-character-lock="accepted_static_master_additive_motion_plates"><section class="master"><img class="studio-stage" src="${assets.canonStageDataUrl}" alt=""><div class="studio-grade"></div>${input.waveformAmplitude === undefined ? "" : `<div class="audio-waveform-reactive"><img class="source-plate" src="${assets.canonStageDataUrl}" alt=""></div>`}<div class="brand-reset"></div><div class="right-reset"></div><div class="bottom-reset"></div><div class="motion-plate neck-plate"><img class="source-plate" src="${assets.canonStageDataUrl}" alt=""></div>${renderVoxyHeadRelativeFaceRig({ canonStageDataUrl: assets.canonStageDataUrl, frame: headFrame })}<div class="motion-plate left-hand-plate"><img class="source-plate" src="${assets.canonStageDataUrl}" alt=""></div><div class="motion-plate right-hand-plate"><img class="source-plate" src="${assets.canonStageDataUrl}" alt=""></div><img class="character-mark lapel-pin" src="${assets.lapelPinDataUrl}" alt="VOXY"><img class="character-mark pocket-mark" src="${assets.edebattePocketMarkDataUrl}" alt="eDebatte"><div class="frame"></div><div class="on-air"><i></i>ON AIR</div><section class="brand-lockup"><img src="${assets.studioLockupDataUrl}" alt="VoiceOpenGov eDebatte"></section><section class="editorial-cue" aria-label="Redaktioneller Hinweis"><div class="cue-copy"><small class="cue-kicker">${escapeHtml(state.segment.kicker)}</small><strong class="cue-title">${escapeHtml(state.segment.title)}</strong><span class="cue-role">${escapeHtml(state.segment.brand)} · kontextueller Hinweis</span></div></section><section class="caption-bar"><span class="caption">${escapeHtml(state.segment.caption)}</span></section></section><div class="portrait-title"><strong>${escapeHtml(state.segment.title)}</strong><small>${escapeHtml(state.segment.kicker)}</small></div><div class="portrait-mask"></div><div class="portrait-caption">${escapeHtml(state.segment.caption)}</div></main></body></html>`;
}
