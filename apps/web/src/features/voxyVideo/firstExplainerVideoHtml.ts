...
.on-air{position:absolute;left:42px;top:34px;height:55px;padding:0 18px;display:${portrait ? "none" : "flex"};align-items:center;gap:12px;border:1px solid rgba(216,231,249,.76);border-radius:8px;background:rgba(1,5,16,.74);font-size:21px;font-weight:800;letter-spacing:.1em;box-shadow:0 12px 34px rgba(0,0,0,.28)}
.on-air i{width:13px;height:13px;border-radius:50%;background:#00d9c0;box-shadow:0 0 ${14 + opacity * 8}px rgba(0,217,192,.8)}
.brand-lockup{position:absolute;left:56px;top:154px;width:350px;height:190px;padding:12px 10px;display:${portrait ? "none" : "block"};border-left:3px solid #00d9c0;background:linear-gradient(90deg,rgba(3,13,32,.78),rgba(3,13,32,.16));filter:drop-shadow(0 15px 32px rgba(0,0,0,.25))}
...
.content-rail{position:absolute;left:1362px;top:54px;width:510px;height:650px;padding:34px 29px;display:${portrait ? "none" : "block"};background:linear-gradient(145deg,rgba(2,10,27,.97),rgba(1,5,17,.92));border-left:1px solid rgba(73,167,255,.58);border-block:1px solid rgba(107,150,205,.34);box-shadow:-24px 0 55px rgba(0,0,0,.3),inset 20px 0 35px rgba(30,107,255,.04)}
...
.caption-bar{position:absolute;left:50px;right:50px;top:798px;min-height:154px;display:${portrait ? "none" : "flex"};align-items:center;padding:22px 46px;background:linear-gradient(90deg,rgba(2,12,31,.985),rgba(2,10,25,.94) 78%,rgba(2,10,25,.86));border-block:1px solid rgba(104,155,215,.5);box-shadow:0 20px 60px rgba(0,0,0,.25)}.caption-bar:before{content:"";position:absolute;left:16px;top:22px;bottom:22px;width:5px;border-radius:4px;background:linear-gradient(#00d9c0,#1e6bff)}.caption{max-width:1500px;font-size:34px;font-weight:700;line-height:1.24;letter-spacing:.008em;opacity:${opacity};transform:translateY(${Math.round((1 - opacity) * 10)}px)}
.footer{position:absolute;left:50px;right:50px;bottom:27px;display:${portrait ? "none" : "flex"};justify-content:space-between;align-items:center;color:#6997cb;font-size:13px;letter-spacing:.14em}.footer b{color:#3bdde5}.portrait-mask{display:${portrait ? "block" : "none"};position:absolute;z-index:10;left:0;right:0;bottom:0;height:150px;background:linear-gradient(transparent,rgba(1,5,17,.98) 40%)}
...
.portrait-title{display:${portrait ? "block" : "none"};position:absolute;z-index:11;left:34px;top:34px;padding:13px 17px;border-left:4px solid #00d9c0;background:rgba(2,10,27,.88);box-shadow:0 12px 30px rgba(0,0,0,.28)}.portrait-title strong{display:block;font-size:23px;letter-spacing:.03em}.portrait-title small{display:block;margin-top:6px;color:#8fb4dd;font-size:12px;letter-spacing:.08em}
</style>
</head>
<body><main class="viewport" data-format="${format}" data-frame-index="${frameIndex}" data-at-ms="${atMs.toFixed(3)}" data-motion-state="${segment.motionState}" data-waveform-count="1" data-waveform-placement="behind_voxy" data-hand-gesture="none_flattened_master_identity_lock">
  ...
  <div class="portrait-title"><strong>${escapeHtml(segment.editorialTitle)}</strong><small>${escapeHtml(segment.editorialRole)}</small></div><div class="portrait-mask"></div><div class="portrait-caption">${escapeHtml(segment.caption)}</div>
</main></body></html>`;
}
