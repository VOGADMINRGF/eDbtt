...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    #fixture { --capture-time:${captureTimeMs}ms; position: relative; width: ${plan.width}px; height: ${plan.height}px; overflow: hidden; isolation: isolate; background: radial-gradient(circle at 50% 42%, #10275f 0%, #050b21 52%, #01040d 100%); }
    ...
    .studio-layer { z-index: 1; filter: saturate(.94) contrast(1.03) brightness(.9); animation: studio-push ${plan.durationMs}ms linear calc(0ms - var(--capture-time)) both; }
    ...
    .waveform span { width: 2.1%; min-width: 2px; height: var(--bar-height); border-radius: 999px; background: linear-gradient(#5aa0ff, #1161ff); box-shadow: 0 0 10px rgba(45,118,255,.65); animation: wave 820ms ease-in-out calc(var(--bar-delay) - var(--capture-time)) infinite alternate; }
    .scene-stack { position: absolute; z-index: 8; top: 16%; right: 3%; width: min(31%, 390px); height: 58%; }
    .scene-card { position: absolute; inset-inline: 0; top: 50%; transform: translate3d(44px,-50%,0) scale(.98); opacity: 0; border: 1px solid rgba(66,137,255,.82); border-radius: 18px; padding: clamp(16px, 2.1vw, 26px); background: linear-gradient(145deg, rgba(3,11,31,.93), rgba(7,20,54,.86)); box-shadow: 0 20px 50px rgba(0,0,0,.38), 0 0 34px rgba(28,98,255,.16); animation: card-cycle var(--scene-duration) ease-in-out calc(var(--scene-start) - var(--capture-time)) both; }
    ...
    .capture-mode .studio-layer, .capture-mode .character-layer, .capture-mode .on-air, .capture-mode .waveform span, .capture-mode .scene-card, .capture-mode .lower-third { animation-play-state: paused !important; }
...
<body>
  <main id="fixture" class="${captureClass}" aria-label="Voxy Character Motion Fixture">
