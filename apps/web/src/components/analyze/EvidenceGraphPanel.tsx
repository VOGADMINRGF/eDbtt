  ...
  return (
    <div className="rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-3 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div>
          <div className="text-[11px] font-semibold uppercase tracking-wide text-[rgb(var(--muted))]">EvidenceGraph</div>
      ...
      <div className="mt-3 space-y-2">
        {claims.slice(0, 8).map((c) => {
          ...
          return (
            <div key={c.id} className="rounded-lg border border-[rgb(var(--border))] bg-[rgb(var(--card))] px-3 py-2">
              <div className="flex items-start justify-between gap-2">
                <div className="text-xs text-[rgb(var(--fg))]">{c.label}</div>
                <span
                  className={
      ...
      {evidence.length > 0 && (
        <div className="mt-3">
          <div className="text-xs font-semibold text-[rgb(var(--muted))]">Top Evidence Nodes</div>
          <ul className="mt-1 space-y-1 text-[11px] text-[rgb(var(--muted))]">
            {evidence.slice(0, 6).map((ev) => (
              <li key={ev.id}>
                <span className="font-medium text-[rgb(var(--muted))]">{ev.publisher || ev.sourceClass || "Quelle"}</span>
