...
export default function ClaimMatchBadge({ match }:{ match: ClaimMatch }){
  if(match.kind==="verified"){
    const s=match.stmt;
    return <div className="flex flex-wrap items-center gap-2 text-xs text-[rgb(var(--muted))]">
  ...
  return <div className="text-xs text-[rgb(var(--muted))]"><span className="vog-chip">∅ Neu</span></div>;
}
