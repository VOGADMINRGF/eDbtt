...
const MOTIONS: VoxyCharacterMotion[] = ["neutral_idle", "listening", "explaining", "questioning", "highlighting_source", "showing_contrast", "inviting_participation"];
...
async function main(): Promise<void> {
  ...
  for (const layer of master.layers) {
    ...
    layerAssets.push({ id: layer.id, path: layer.sourcePath, sha256: await sha256(file), url: await dataUrl(file), pivot: layer.pivot, zIndex: layer.zIndex });
  }

  const browser = await chromium.launch({ headless: true });
  const context = await browser.newContext({ viewport: { width: WIDTH, height: HEIGHT } });
  const page = await context.newPage();
  const layerMarkup = layerAssets.map((layer) => `<img class="rig-layer" data-layer-id="${layer.id}" src="${layer.url}" alt="" style="z-index:${layer.zIndex};transform-origin:${layer.pivot ? `${(layer.pivot.x / 1600) * 100}% ${(layer.pivot.y / 1600) * 100}%` : "50% 50%"}">`).join("");
  await page.setContent(`<!doctype html><html><head><meta charset="utf-8"><style>*{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:#020718}#stage{position:relative;width:${WIDTH}px;height:${HEIGHT}px;overflow:hidden;background:radial-gradient(circle at 52% 36%,#102855,#020718 70%)}#rig{position:absolute;width:720px;height:720px;left:280px;top:0}.rig-layer{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;will-change:transform,opacity}.safe-area{position:absolute;display:none;border:3px dashed #25e6ff;border-radius:12px;z-index:999;pointer-events:none;box-shadow:0 0 0 1px rgba(2,7,24,.8)}</style></head><body><main id="stage"><div id="rig">${layerMarkup}</div><div id="safe" class="safe-area"></div></main></body></html>`, { waitUntil: "load" });
  await page.waitForFunction(() => Array.from(document.images).every((image) => image.complete));

  const applyFrame = async (timeMs: number) => {
    const motionIndex = Math.min(MOTIONS.length - 1, Math.floor((timeMs / DURATION_MS) * MOTIONS.length));
    const frame = buildVoxyMasterMotionFrame({ master, motion: MOTIONS[motionIndex], timeMs });
    const transforms = Object.fromEntries(frame.transforms.map((item) => [item.layerId, { transform: transformCss(item), opacity: item.opacity }]));
  ...
  const safe = async (left: number, top: number, width: number, height: number) => page.evaluate(({ left, top, width, height }) => {
  ...
  for (const spec of cropSpecs) {
    await safe(spec.x + spec.marginX, spec.y + spec.marginY, spec.width - spec.marginX * 2, spec.height - spec.marginY * 2);
    ...
    cropEvidence.push({ format: spec.format, path: path.replace(`${process.cwd()}/`, ""), sha256: await sha256(path), width: spec.width, height: spec.height, safeAreaVisible: true });
  }
  ...
  const manifest = {
    ...
    layerAssets: layerAssets.map(({ id, path, sha256: hash, pivot, zIndex }) => ({ id, path, sha256: hash, pivot, zIndex })),
    render: { path: renderPath.replace(`${process.cwd()}/`, ""), sha256: await sha256(renderPath), durationSeconds, fps: FPS, width: Number(probe.streams?.[0]?.width ?? WIDTH), height: Number(probe.streams?.[0]?.height ?? HEIGHT), frameCount: FRAME_COUNT },
    cropEvidence,
  ...
  console.log(JSON.stringify({ status: "voxy_layered_render_ready_for_human_review", commitSha, render: manifest.render, cropEvidence, layerCount: manifest.layerAssets.length }, null, 2));
}

main().catch((error: unknown) => {
  console.error(`VOXY_ANIMATABLE_RENDER_FAILED: ${error instanceof Error ? error.message : "unknown_error"}`);
  process.exitCode = 1;
});
