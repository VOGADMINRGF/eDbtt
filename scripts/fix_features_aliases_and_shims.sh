...
########################################
CLARIFY="$ROOT/features/analyze/clarify.ts"
if [ ! -f "$CLARIFY" ]; then
  mkdir -p "$(dirname "$CLARIFY")"
  cat > "$CLARIFY" <<'TS'
export function clarifyForPrices() {
  return {
    ask: [
      "Bitte präzisieren: Welche Preise genau?",
      "Konkreter = bessere Zuordnung, Faktencheck, Quellen."
    ],
    options: [
      { key: "opt1", label: "Lebensmittelpreise" },
      { key: "opt2", label: "Energiepreise" },
      { key: "opt3", label: "Kraftstoffpreise" },
      { key: "opt4", label: "Mieten/Nebenkosten" },
      { key: "opt5", label: "ÖPNV/Telekom-Tarife" }
    ],
    quickSources: [
      { label: "Destatis", url: "https://www.destatis.de" },
      { label: "Bundeskartellamt", url: "https://www.bundeskartellamt.de" }
    ]
  };
}
...
export function needsClarify(input: { text?: string; categoryMain?: string|null; region?: string|null }) {
  ...
  const cat = String(input?.categoryMain || "").toLowerCase();
  const generic = /preis(e|steigerung|erhoehung|erhöhung)/.test(t) && !/(lebensmittel|energie|kraftstoff|miete|tarif|telekom|oepnv|öpnv)/.test(t);
  const catGeneric = !cat || /preis/.test(cat);
  return generic || catGeneric;
}
TS
  echo "  • wrote fallback: features/analyze/clarify.ts"
fi

########################################
# D) Build-Cache leeren
########################################
