...
########################################
# A) Unicode/Folding-Helfer
########################################
cat > "$SRC/lib/text/normalize.ts" <<'TS'
/**
 * Fold/normalize for search & matching (no i18n framework).
 * - NFKD + remove combining marks
 * - lowercasing
 * - common German/Latin folds
 * - collapse whitespace
 */
...
export async function GET(req: NextRequest) {
  ...
  // Beispiel 1: verifiziertes Match (Touristen/Abzocke)
  ...
  // Beispiel 2: Cluster-Match (ÖPNV/Tram/Strassenbahn …)
...
########################################
# C) /api/translate (GPT-only, ohne i18n)
########################################
...
export async function POST(req: NextRequest) {
  ...
  // Kosten & Robustheit: weich limitieren (kannst du erhöhen)
  ...
  const out = completion.choices[0]?.message?.content?.trim() ?? clipped;
  setCache(key, out);
...
########################################
# D) Client-Helper (optional, bequem)
########################################
