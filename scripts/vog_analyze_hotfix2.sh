...
# ===== 1) Vollständige, verbesserte Heuristik =====
cat > "$root/features/analyze/wrapper.ts" <<'TS'
import { z } from "zod";
...
# ===== 2) Ergebnis-UI: Zeige auch bei 0 Claims etwas Verständliches =====
pageAnalyze="$root/apps/web/src/app/contributions/analyze/page.tsx"
if [ -f "$pageAnalyze" ]; then
  perl -0777 -pe 's/result\?\.\claims\?\.\length \? \(/true ? (/g' -i "$pageAnalyze" || true
fi

echo "✓ Hotfix eingespielt. Starte/refresh @vog/web und teste erneut."
