...
head = head.replace(
    ...
    "- Letzter gezielter Kopf-Sync: Marketing Growth OS und fokussierte Admin-Informationsarchitektur nach PR `#443`; nicht berührte operative Zeilen behalten ihren zuletzt verifizierten Status.",
    1,
)
head = head.replace(
...
truth_block = (
    ...
    "- PR `#460` / Issue `#458` überarbeitet ausschließlich die Informationsarchitektur und Bedienbarkeit von `/admin/marketing`; keine neue Queue, Agentenruntime oder globale Workspace-Architektur.\n"
...
| ADMIN-MARKETING-OPERATING-DASHBOARD-02 | in_progress | P1 | MARKETING-REGISTRY-READMODEL-01 | Issue `#458` / PR `#460`: `/admin/marketing` als verständliche Betreiberübersicht mit priorisierten, klickbaren Kennzahlen, echten Ergebnissen, verständlichen nächsten Schritten und Übergaben in vorhandene Arbeitsbereiche überarbeiten | Scope bleibt lokal auf `/admin/marketing`, Registry-Readmodel, Navigation und fokussierte Tests; keine Demo-Werte, neue Queue, Agentenruntime, Root-Layout-, Token-, Shared-Component-, `/create`-, `/runden`- oder `/dossier`-Änderung |
