...
Status: prepared / non-destructive foundation
Issue: #728
...
- Unter `prisma/web/` und `prisma/core/` existiert bislang jeweils kein kanonischer `migrations/`-Verlauf.
...
1. Baseline-SQL für Web und Core lokal mit Node 20 + Prisma 6.16.3 erzeugen.
2. Fresh-DB-Reproduzierbarkeit für beide Schemas beweisen.
3. Legacy-Historie als nicht deploybar dokumentieren.
4. Preview-DB schema-only inventarisieren.
5. Erst danach Brownfield-Adoption planen.
6. Production bleibt bis Human-Gate unangetastet.
