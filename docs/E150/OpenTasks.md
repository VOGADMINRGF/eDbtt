...
| GOV-AI-ORCH-05 | codex_ready | high | GOV-AI-ORCH-03, GOV-AI-04 | E150 Lane-Split + Journey-Defaults + sealed Factcheck Contract | Verbindliche Zwei-Lane-Architektur (`standard` vs `sealed_factcheck`) technisch in Orchestrierung/Analyze/Factcheck umsetzen | Journey-Profile + role routing sind aktiv; Standard-Lanes defaulten auf `researchUsed=none`; sealed Factcheck ist exklusiver Research-/Siegelpfad (`verificationMode=sealed`); neue Contract-Felder (`verificationMode`, `researchUsed`, `sealEligible`, `sealGranted`) sind im Antwort-Meta sichtbar; OpenAI bleibt auf `fallback` + optionalen non-mutativen `presentation_pass` begrenzt; winner-takes-all gilt explizit als Altmodell und ist im Zielrouting abgeloest; Nutzerlabels `analysiert`/`geprueft`/`verifiziert` sind contract-scharf gemappt; Tests fuer routing/fallback/contract sind gruen | no | Neu (2026-04-23): verbindliche Zielarchitektur festgeschrieben in `docs/E150/GOV-AI-ORCH-05_LANE_SPLIT_SEALED_FACTCHECK_ARCHITECTURE_2026-04-23.md`; dieser Task ist der erste technische Umsetzungsslice. |
...
1. `GOV-AI-ORCH-05` - E150 Lane-Split + Journey-Defaults + sealed Factcheck Contract.
