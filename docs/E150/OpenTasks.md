...
| PR-AI-PRICING-64 | done | high | #58, #59, PR-AI-RESEARCH-55 | Pricing alignment with research credits and premium search add-ons (Issue #64) | Preis-/Paketmodell so nachschärfen, dass externe Recherche optional bleibt und Credits/Add-ons klar getrennt sind, ohne Standard-Analyze-Abhängigkeit auf Perplexity/ARI | `/pricing` kommuniziert klar: Standard Analyze läuft ohne externe Research-Provider; Perplexity Search/ARI Deep Research sind explizite optionale Credits/Add-ons; private Paketpreise und Pakethinweise sind auf `3,99`/`9,99`/`29,99` inkl. Contribution-/Anlassraum-/Credit-Logik ausgerichtet; zentrale Pakettypen enthalten Entitlement-Felder (`contributionCreditsPerMonth`, `anlassraumCredits`, `searchCredits`, `deepResearchCredits`, `dossierBoostEligible`, `premiumResearchEligible`); unbekannte Providerkosten bleiben in Docs als `unknown`/`n/a` statt `0 EUR`; Segmentpfade (`?segment=journalismus` etc.) führen Mitgliedschaft als optionalen Antrag ohne automatischen Segmentrabatt; keine Auto-Aktivierung von Perplexity/ARI | no | Erledigt (2026-04-29): Modellfelder in `features/pricing/domain/types.ts`; Paketdaten in `features/pricing/domain/{plans.de.ts,plans.en.ts}`; Copy-Härtung in `apps/web/src/app/{pricing/page.tsx,vormerken/page.tsx}` und `features/pricing/domain/content.de.ts`; Dokumentation in `docs/E150/{Part19_Pricing_Packaging.md,membership_pricing.md}`; Contract-Preis-Expectations aktualisiert in `apps/web/tests/{pricing-page.contract.test.ts,pricing-private-package-prices.contract.test.ts,vormerken-private-package-prices.contract.test.tsx,pricing-i18n.contract.test.ts,vormerken-page.contract.test.tsx,no-legacy-price-logic.contract.test.tsx,pricing-private-member-price.contract.test.ts}`. |
...
1. `PR-AI-ORCH-POLICY-01` — Graph-guided policy orchestration authority (Referenzen: #58, #48, #49, #55).
