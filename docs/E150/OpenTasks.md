...
| PR-PRICING-B2B-PARTNER-01 | codex_ready | high | PR-PRICING-B2G-VERGABE-01 | B2B-Partnerfokus fuer Beteiligungsbueros, Moderationsbueros und Dialogprofis auf `/pricing/institutionen` | B2B klar als Werkzeug-/Infrastrukturpartner fuer Beteiligungsprofis positionieren, getrennt von B2G-Vergabepaketen | Segment `organisationen` zeigt explizite Zielgruppen (Beteiligungsbueros, Moderationsbueros, Planungsbueros, Kommunikationsagenturen, Dialogberatungen, Stiftungs-/Demokratieprogramme, Medien-/Community-Partner); vier B2B-Paketstufen (`Starter`, `Projektpartner`, `Buero-Betrieb`, `Partner-/Rahmenmodell`) mit Zweck/Leistungen/Ergebnis/Bestellbarkeit/CTA; keine Behauptung `ersetzt Bueros`; keine Fake-White-Label-Zusage; CTAs fuehren mit `segment=organisationen` nach `/order` | no | Follow-up-Slice laut Produktpriorisierung (2026-04-30) vorbereitet; eigene Doku `docs/E150/pricing-b2b-partner.md` im Slice anlegen. |
| PR-LANDING-CLARITY-01 | codex_ready | high | PR-PRICING-B2B-PARTNER-01 | Startseite auf 3-Sekunden-Verstaendnis mit Segment-Einstiegen schaerfen, ohne LandingAssistant-/Marquee-Rueckbau | Besucher sollen sofort verstehen, was eDebatte und VoiceOpenGov sind und welche naechsten Einstiege es gibt | Hero-Claim + Subline erklaeren Dossier-/Beteiligungslogik in einem Blick; vier Einstiegskarten (`Buerger:innen`, `Kommunen`, `Beteiligungsbueros`, `Journalist:innen`) sichtbar; Prozesszeile `Signal -> Dossier -> Runde -> Mandat -> Umsetzung` sichtbar; LandingAssistant bleibt funktional; CTAs fuehren auf vorhandene Routen (`/create`, `/pricing?segment=kommunen`, `/pricing/institutionen`, Dossier-Beispiel) | no | Follow-up-Slice laut Produktpriorisierung (2026-04-30) vorbereitet; Evidence-Datei nach Umsetzung anlegen. |
...
| PR-OUT-ENGINE-05 | codex_ready | high | PR-OUT-ENGINE-04 | Format Mapper Layer (Article/Briefing/Letter/Administrative Note) | Stub-Ausgaben in kanalnahe Textformate ueberfuehren, weiterhin review-gated | Deterministische Mapper je Format; Quellen-/Unsicherheitsabschnitt bleibt erhalten; kein Auto-Publish | no | Lokalstand `implemented_local` (2026-04-30): erster konkreter Format-Mapper `social_carousel` in `features/outputEngine/socialCarousel.ts` inkl. Varianten-Metadaten (`square`, `story`, `linkedin`, `print_preview`) und visuellem Studio-Renderer `apps/web/src/components/outputEngine/SocialCarouselPreview.tsx`; Post-ready Verfeinerung lokal ergaenzt (deterministische Posting-Metadaten, regionales Framing mit Fallback, Guardrails `canAutoPublish=false`, `publicationStatus=draft_review_required`); Distribution-Planung lokal erweitert (`features/outputEngine/socialDistribution.ts`, Studio-Panel `apps/web/src/components/outputEngine/SocialDistributionPanel.tsx`) mit kanalweiser Connector-/Policy-Anzeige, Scheduling-Optionen und no-live-publish Guardrails (`externalApisEnabled=false`, `canRealtimePublish=false`); UX-Refactor lokal als Publishing-Cockpit umgesetzt (Veröffentlichungs-Paket, kanalorientierte Post-Karten, Queue/Verteilplan, kollabierte `Dossier-Qualität & Hinweise`, technische Details de-emphasized); Master-Post-first Studio-Hardening lokal ergänzt (Dossier-Post als primäres Objekt, Hauptaktionen mit lokalem State/Clipboard/localStorage, `Kanäle auswählen` + `Kanalverbindungen` + `Veröffentlichungsmodus` + `Verteilplan übernehmen`, Kanal-Versionen als sekundäre Ableitungen, explizite Guardrails ohne externe Live-Veröffentlichung). Dieser Slice ist explizit **nicht** Beteiligungsradar/Signal-Ingestion/Anlassraum-Automation. Tests: `apps/web/tests/output-engine-social-carousel.test.ts`, `apps/web/tests/output-engine-social-distribution.test.ts`, `apps/web/tests/dossier-output-studio.page.contract.test.ts`. Kein Done/kein Merge, Folge-Slices 06-09 bleiben offen. |
...
1. `PR-OUT-ENGINE-05` — Format Mapper Layer (Article/Briefing/Letter/Administrative Note).
2. `PR-AI-ORCH-POLICY-01` — Graph-guided policy orchestration authority (Referenzen: #58, #48, #49, #55).
3. Operative Priorisierung `Jetzt / Danach / Parken`: `docs/E150/CURRENT_STATE_2026-04-04.md`.
