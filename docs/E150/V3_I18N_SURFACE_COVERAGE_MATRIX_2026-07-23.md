...
| FAQ | `/faq` | routeeigene Page | JSX, AT | `de`, teils `AT-6` | ja | nein | routeeigene Übersetzung statt Bundle-SSOT | ja | nein | nicht fokussiert belegt | nein | indexierbar | teilweise | Fragen, Antworten und Akkordeons brauchen explizite RTL-/Mobile-Abnahme |
| Kontakt | `/kontakt` | routeeigene Page | JSX, AT | `de`, teils `AT-6` | ja | nein | Kontaktcopy und Übersetzungswiring nicht bundlegetrieben | ja | nein | nicht fokussiert belegt | nein | indexierbar | teilweise | Form-, Kontakt- und Fehlermeldungen in definierter UI-Sprache prüfen |
| Über uns | `/ueber-uns` | routeeigene Page | JSX, AT | `de`, teils `AT-6` | ja | nein | öffentliche Longform ohne systemweites Bundle-SSOT | ja | nein | nicht fokussiert belegt | nein | indexierbar | teilweise | Longform, CTA und Navigation brauchen RTL-/Mobile-Beleg |
| Transparenzbericht | `/transparenzbericht` | routeeigene Page | JSX, AT | `de`, teils `AT-6` | ja | nein | öffentliche Transparenzcopy ohne flächigen Locale-Vertrag | ja | nein | nicht fokussiert belegt | nein | indexierbar | teilweise | Bericht, Tabellen und Quellenhinweise explizit prüfen |
| Verhaltenskodex | `/verhaltenskodex` | routeeigene Page | JSX, AT | `de`, teils `AT-6` | ja | nein | öffentliche Regelcopy ohne Bundle-SSOT | ja | nein | nicht fokussiert belegt | nein | indexierbar | teilweise | Regeln und Meldewege dürfen keinen stillen Sprachwechsel enthalten |
| Mitglied werden | `/mitglied-werden` | routeeigene Page | JSX, AT | `de`, teils `AT-6` | ja | nein | öffentliche Membership-Journey nicht systemweit lokalisiert | ja | nein | nicht fokussiert belegt | nein | indexierbar | teilweise | Nutzen, CTA, Form-Handoff und Bedingungen sprachlich deckungsgleich prüfen |
| How-to-works | `/howtoworks/*` | routeeigene How-to-Pages | JSX, AT | routeabhängig `de`, teils `AT-6` | ja | nein | mehrere öffentliche Erklärflächen ohne gemeinsame Bundle-Wahrheit | ja | nein | nicht fokussiert belegt | nein | indexierbar | teilweise | jede How-to-Route braucht eigene Fallback-, RTL- und Mobile-Abnahme |
| Login | `/login` | `LoginPageShell` | JSX, OP | `de` | ja | nein | private Route ohne Auto-Übersetzung; keine robots-Meta | ja | nein | getestet | nein | indexierbar (keine robots-Meta) | nicht lokalisiert | Formcopy und Discovery-/Privacy-Lücke bleiben explizit ausgewiesen |
| Registrierung | `/register` | `RegisterPageClient` | JSX, OP | `de` | ja | nein | mehrstufige Inline-Form; keine robots-Meta | ja | nein | teilweise | nein | indexierbar (keine robots-Meta) | nicht lokalisiert | Schritt-Texte, Validierung und Discovery-Grenze separat schließen |
| Passwort-Reset | `/reset` | `ResetPage` | JSX | `de` | ja | nein | reine Inline-DE-Copy; keine robots-Meta | ja | nein | nein | nein | indexierbar (keine robots-Meta) | nicht lokalisiert | Request-/Setzen-Flows und Discovery-Grenze explizit absichern |
| Verifikation | `/verify` | `VerifyPage` | JSX | `de` | ja | nein | Inline-DE inkl. Fehlermeldungen; keine robots-Meta | ja | nein | nein | nein | indexierbar (keine robots-Meta) | nicht lokalisiert | Verify-/Resend-Texte und Discovery-Grenze konsistent schließen |
...
| Live-Root | `/live` | `LivePage` | JSX | `de` intern | ja | nein | env-/admin-gated Stub | ja | nein | getestet | nein | intern/nicht nutzerseitig | intern/nicht nutzerseitig | solange Stub: keine Mehrsprachigkeitsbehauptung |
| Live-Campaign-Entry | `/live/[campaignId]` | `LiveCampaignEntryClient` | JSX, OP, EXT | überwiegend `de` | ja | nein | öffentliche Campaign-/QR-Entry-Surface ohne belegte Bundle-Nutzung | ja | nein | getestet | nein | indexierbar | nicht lokalisiert | öffentliche Entry-Copy, CTA und QR-Kontext brauchen explizite UI-, RTL- und Mobile-Abnahme |
...
### Ergänzte Review-Evidenz

- `/faq`, `/kontakt`, `/ueber-uns`, `/transparenzbericht`, `/verhaltenskodex`, `/mitglied-werden` und `/howtoworks/*` sind als eigenständige öffentliche Flächen inventarisiert.
- `/login`, `/register`, `/reset` und `/verify` sind im aktuellen Iststand mangels robots-Meta indexierbar.
- `/live` bleibt ein interner Stub; `/live/[campaignId]` ist dagegen eine öffentliche Campaign-/QR-Entry-Surface.
