    ...
    const userMsg: ChatMessage = {
      ...
      content: (
        <div className="space-y-1">
          <p className="font-semibold text-[rgb(var(--fg))]">Dein Input</p>
          <p className="text-sm text-[rgb(var(--muted))] whitespace-pre-wrap">{text}</p>
          {preparedContext && (
            <p className="text-[11px] text-[rgb(var(--muted))]">Kontext: {preparedContext}</p>
          )}
        </div>
      ),
    };
    ...
    <section className="mx-auto max-w-5xl space-y-6 bg-gradient-to-b from-slate-50 via-white to-emerald-50 px-4 pb-16 pt-10">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_0.9fr]">
        <div className="space-y-4">
          <div className="rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-xs font-semibold uppercase tracking-wide text-[rgb(var(--muted))]">Nachricht an eDebatte</p>
                <p className="text-[12px] text-[rgb(var(--muted))]">Keine Wiederholung im Output, keine Wahlempfehlungen.</p>
              </div>
            ...
            <textarea
              className="w-full rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))] px-4 py-3 text-sm text-[rgb(var(--fg))] outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
              rows={6}
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              ...
              {lastResult && (
                <button
                  ...
                  disabled={loading || !text.trim()}
                  className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-[rgb(var(--card))] px-4 py-2 text-sm font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-60"
                >
          ...
          <div className="space-y-3 rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[rgb(var(--muted))]">Onboarding (optional)</p>
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-2">
                <p className="text-[12px] font-semibold text-[rgb(var(--muted))]">Rolle</p>
                    ...
                    <button
                      ...
                      className={`rounded-full px-3 py-1 text-xs font-semibold ${
                        role === opt
                          ...
                          : "bg-[rgb(var(--bg))] text-[rgb(var(--muted))] border border-[rgb(var(--border))]"
                      }`}
                    >
                      {opt}
                    </button>
              ...
              <div className="space-y-2">
                <p className="text-[12px] font-semibold text-[rgb(var(--muted))]">Ebene</p>
                <div className="flex flex-wrap gap-2">
            ...
            <div className="space-y-2">
              <p className="text-[12px] font-semibold text-[rgb(var(--muted))]">Fokus-Aspekte</p>
              <div className="flex flex-wrap gap-2">
          ...
          <div className="space-y-3 rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4 shadow-sm">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[rgb(var(--muted))]">Verlauf</p>
              ...
              <div className="mt-2 space-y-2 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <a
                    ...
                    className="rounded-full border border-emerald-200 bg-[rgb(var(--card))] px-4 py-2 text-xs font-semibold text-emerald-800 hover:bg-emerald-50"
                  >
                    Dossier vertiefen
                  </a>
                  <button
        ...
        <aside className="space-y-3">
          <div className="rounded-3xl border border-emerald-100 bg-[rgb(var(--card))] p-4 shadow-sm">
          ...
          {lastResult && (
            <div className="rounded-3xl border border-[rgb(var(--border))] bg-[rgb(var(--card))] p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[rgb(var(--muted))]">Letzter Check (Kurz)</p>
              <ul className="mt-2 space-y-2 text-sm text-[rgb(var(--muted))]">
              ...
              <p className="mt-3 text-[11px] text-[rgb(var(--muted))]">Details im Verlauf links.</p>
            </div>
          )}
        </aside>
      </div>
  ...
  return (
    <div className="space-y-3">
      <header className="flex items-center justify-between">
        ...
        <span className="rounded-full bg-[rgb(var(--card))] px-2 py-1 text-[11px] font-semibold text-emerald-800">
          Check → Dossier → Beteiligung
        </span>
      </header>
      ...
      <Section title="Check" badge="Claims">
        <div className="space-y-2">
          ...
          {claims.map((c, idx) => (
            <div key={c.id ?? idx} className="rounded-xl border border-emerald-100 bg-[rgb(var(--card))] p-3 text-sm text-emerald-900">
          ...
          <div className="space-y-3">
            {questions.length > 0 && (
              <Block title="Offene Fragen">
                <ul className="space-y-2">
                  {questions.map((q, idx) => (
                    <li key={q.id ?? idx} className="rounded-lg bg-[rgb(var(--card))] p-2 text-sm text-emerald-900">
                      {q.text}
                      {q.dimension && (
            ...
            {missing.length > 0 && (
              <Block title="Fehlende Perspektiven / Bias">
                <ul className="space-y-2">
                  {missing.map((p, idx) => (
                    <li key={p.id ?? idx} className="rounded-lg bg-[rgb(var(--card))] p-2 text-sm text-emerald-900">
                      {p.text}
                      {p.dimension && (
            ...
            {knots.length > 0 && (
              <Block title="Spannungsfelder">
                <ul className="space-y-2">
                  {knots.map((k, idx) => (
                    <li key={k.id ?? idx} className="rounded-lg bg-[rgb(var(--card))] p-2 text-sm text-emerald-900">
            ...
            {notes.length > 0 && (
              <Block title="Kontext & Hinweise">
                <ul className="space-y-2">
                  {notes.map((n, idx) => (
                    <li key={n.id ?? idx} className="rounded-lg bg-[rgb(var(--card))] p-2 text-sm text-emerald-900">
      ...
      <Section title="Beteiligung" badge="Abstimmbar">
        <div className="space-y-2">
          {participationItems.map((item, idx) => (
            <div
              key={item.id ?? idx}
              className="rounded-xl border border-emerald-100 bg-[rgb(var(--card))] p-3 text-sm text-emerald-900"
            >
...
function Section({ title, badge, children }: { title: string; badge?: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-emerald-100 bg-[rgb(var(--card))] p-3">
      <div className="flex items-center gap-2">
