...
import {
  ...
  validateFactcheck,
} from "@core/utils/validation/promptSchemas";
...
// ————————————————————————————————————————————————————————————————
// Public re-exports (Atomicizer bleibt optional nutzbar)
export { atomicize, type AtomicizeOpts, type AtomicClaim } from "./roles/atomicizer";
...
export function healthScore(id: string) {
  ...
  const p95 = m.p95ms.length ? m.p95ms.toSorted((a, b) => a - b)[Math.max(0, Math.floor(m.p95ms.length * 0.95) - 1)] : 2000;
...
export function beforeCall(id: string) {
  const m = (METRICS[id] ??= { ok: 0, fail: 0, jsonOk: 0, p95ms: [], state: "closed" });
  if (m.state === "open") {
    if (!m.openedAt || Date.now() - m.openedAt < 15_000) throw new Error(`cb-open:${id}`);
    m.state = "half";
  }
}
...
export function afterCall(id: string, ms: number, ok: boolean, jsonValid: boolean) {
  const m = (METRICS[id] ??= { ok: 0, fail: 0, jsonOk: 0, p95ms: [], state: "closed" });
  ok ? m.ok++ : m.fail++;
  if (jsonValid) m.jsonOk++;
  m.p95ms.push(ms);
  if (m.p95ms.length > 100) m.p95ms.shift();

  if (!ok) {
    if (m.state === "half" || m.fail > 3) {
      m.state = "open";
      m.openedAt = Date.now();
    }
  } else if (m.state !== "closed") {
    m.state = "closed";
    m.openedAt = undefined;
  }
}

export function sortProvidersByHealth(
  entries: Array<[ProviderId, { enabled?: () => boolean }]>
) {
  return entries.filter(([, p]) => p.enabled?.()).sort((a, b) => healthScore(b[0]) - healthScore(a[0]));
}
...
const DEFAULT_PROVIDER_ORDER: ProviderId[] = (
  process.env.AI_PROVIDER_ORDER?.split(",").map(s => s.trim().toLowerCase() as ProviderId) ??
  ["openai", "anthropic", "mistral", "gemini"]
).filter(Boolean);

const PROVIDER_TIMEOUTS: Record<ProviderId, number> = {
  openai: Number(process.env.OPENAI_TIMEOUT_MS ?? 18_000),
  anthropic: Number(process.env.ANTHROPIC_TIMEOUT_MS ?? 22_000),
  mistral: Number(process.env.MISTRAL_TIMEOUT_MS ?? 18_000),
  gemini: Number(process.env.GEMINI_TIMEOUT_MS ?? 18_000),
};

// Budget guard (ms) pro orchestriertem Task
const BUDGET_MS = Number(process.env.AI_BUDGET_MS_DEFAULT ?? 35_000);
...
async function injectPartials(text: string) {
  ...
  if (!partialRe.test(text)) return text;
  const partial = await loadPrompt("_shared_constraints");
  return text.replace(partialRe, partial);
}
...
async function buildPrompt(task: TaskId, vars: Vars) {
  ...
  let tpl = await loadPrompt(nameMap[task]);
...
async function callProvider(provider: ProviderId, prompt: string): Promise<ProviderOk | ProviderFail> {
  const t0 = Date.now();
  ...
  try {
    if (provider === "openai") {
      ...
      return { ok: true, provider, text, raw, latency: Date.now() - t0, stateBefore: before };
    }
...
// Validierung per Task
function validateByTask(task: TaskId, jsonText: string) {
  switch (task) {
    case "impact_only":
...
async function tryOnce(task: TaskId, provider: ProviderId, prompt: string) {
  ...
  if (!r.ok) {
    ...
    await logEvent({
      ...
      latencyMs: r.latency,
      error: r.error,
      circuitBefore: r.stateBefore,
      circuitAfter: Health.get(provider).state,
    });
    throw Object.assign(new Error(r.error), { provider, rawText: undefined as string | undefined });
  }

  const parsedOk = safeJsonParse<any>(r.text).ok;
  if (!parsedOk) {
...
export async function runOrchestratedTask(
  ...
  const basePrompt = await buildPrompt(task, vars);
  ...
  // Hedge: zwei beste parallel (jeweils 1 Guard-Retry)
  const hedge = dynamicOrder.filter(p => Health.canAttempt(p)).slice(0, 2);
  const rest = dynamicOrder.filter(p => !hedge.includes(p));
  ...
  if (hedge.length) {
    try {
      ...
      // weiter zu sequenzieller Phase
    }
  }

  // Sequenziell über Rest (inkl. Hedge-Kandidaten erneut, in Score-Reihenfolge)
  const seq = hedge.concat(rest);
  let lastText: string | undefined;
  let lastProvider: ProviderId | undefined;

  for (const p of seq) {
    if (Date.now() + 2000 >= budgetEnd) break;
    try {
...
// ————————————————————————————————————————————————————————————————
// Drop-in Wrapper, damit deine Route exakt kompiliert:
export async function orchestrateClaim(text: string) {
  // Default: „impact_only“ – bei Bedarf später via Query wählbar
  const res = await runOrchestratedTask("impact_only", { text });
  if (!res.ok) throw new Error(res.error);
  return res.parsed;
}
export async function orchestrateClaimsV2(texts: string[]) {
  return Promise.all(texts.map(t => orchestrateClaim(String(t ?? ""))));
}
export const orchestrateClaimsLegacy = orchestrateClaimsV2;
